...
При помощи интерфейсов можно реализовать и функциональность, эквивалентную 
`(⊸)`, `(¿_)` и `(!_)`:
...
interface Returns<T> {
  fun !return(v : T) : Nothing
}

interface Throws {
  fun !throw(e : Throwable) : Nothing
  fun !discard() nextState(Nothing)
}
```

Можем мы использовать интерфейсы и для typesafe-builder'ов.
