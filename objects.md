...
Будем называть строго-функциональным ядром Kotlin'а вариант языка, все функции представляют из себя чистые функции, а все аргументы, параметры, и все локальные переменные представляют из себя данные. 

В частности:
* исключается использование команд, даже таких как `return`, `throw` и `break`; 
...
Для лучшего соответствия математическим обозначениям, мы также будем использовать
* одинарное равенство `a = b` вместо двойного в качестве оператора равенства;
* оператор присвоения `val a := expr` вместо одинарного равенства в качестве оператора присвоения;
* отбитый пробелом правый треугольник `x ▸ f` вместо `x.let f` в качестве оператора применения функции налево `x ▸ f := f(x)`;
* будем интерпретировать индентированные в точности на два пробела блоки, как будто они заключены в фигурные скобки; 
...
(TODO: написать про питоновые инты, бинарные операторы <name>, тайпклассы, алиасы)
(TODO: написать что локальные переменные можно)
