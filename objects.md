...
Если в языке есть инструкции, заменяемость на значение уже не работает. Для примера предположим, что в нашем языке есть оператор `RAND`, недетерминистски генерирующий случайное целое число. Рассмотрим следующие две программы, которые должны бы быть эквивалентны:
```kotlin
val n := RAND
n + n
```
...
Инструкции внешне похожи на выражения, их контексты могут содержать кроме значений ещё и операторы, такие как вышеприведённые `RAND` или `TRACE`.
