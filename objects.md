...
Теперь допустим, что мы считали значение из ячейки памяти и взвесили яблоко, установив, что `Apple1.weight = 93g` и `X.value = 2`. Упоминающиеся тут яблоко `Apple1` и ячейка памяти `X` — материальные объекты, а вот «целое число 2» и «93 грамма» — платонические “объекты”, существующие исключительно в мире математических абстракций. В их отношении мы будем применять термины данные (мн.ч) и значение (ед.ч). **Данные — информация, отделённая от носителя и контекста**.
...
После того как мы разобрались с определениями и базовыми различиями, нужно разобраться, почему языки программирования гораздо лучше проектировать с учётом этих различий.

## Критерии корректности и корректность-по-построению
-----------------------------------------------------

Для большинства из нас концепция объекта куда более привычна, чем концепция значения — поэтому легко предположить, что значения это какая-то сложная и непонятная математическая абстракция, в то время как объекты просты и наглядны. На деле ситуация обстоит ровно противоположным образом:  
Для описания свойств и взаимоотношений значений, и строгих рассуждений о них математики ещё полтора века назад [Frege1879] разработали формализованный язык, называемый **исчислением предикатов**. Десятилетия теоретических исследований и практического использования исчисления предикатов позволили убедиться в его корректности и достаточной выразительности, понять все его возможности и ограничения; выработался и стабилизировался современный удобный синтаксис. 

Когда говорят о чисто-функциональных (purely functional) языках программирования, часто упоминают, что у них есть “математическая основа“. Что имеется в виду и чем это вообще хорошо?
Чисто-функциональные языки программирования характерны тем, что оперируют напрямую только и исключительно данными, теми самыми для рассуждений о которых имеется развитый математический инструментарий.

Свойства программ можно разделить на операционные и экстенсиональные. Операционные свойства — это свойства, которые могут зависеть от устройства и целевой архитектуры компилятора/интерпретатора языка, например “`sort(list : List<size = n>)` выполняется за `O(n·log n)` редукций. Экстенсиональные свойства — свойства не зависящие от устройства компилятора. К ним относятся все свойства касающиеся корректности работы программы.

Выразительность исчисления предикатов означает, что с его помощью можно выразить любое экстенсиональное свойство программ. Мы можем сформулировать требование, что функция `sort` всегда правильно сортирует, а функция groupBy правильно группирует. Не факт, что любое свойство любой программы легко будет доказать или опровергнуть, но у нас есть гарантия, что мы можем его сформулировать! Более того, логика первого порядка позволяет убедиться, что спецификация полна, и мы не упустили из виду каких-то редких пограничных случаев.

На практике никто не выписывает формально условия корректности каждой функции, но само понимание как это делать, позволяет выработать достоверную ментальную модель — неформальный способ “думать” о программах на строго-функциональных языках. А эта модель позволяет писать корректные-по-построению программы.

К сожалению, ничего из этого в данный момент не доступно в полной мере для императивных языков программирования, а ведь именно это в конечном итоге и является ключом к написанию надёжного софта. Условия корректности каждой функции не выписывают не из-за лени, а потому, что их всё равно никто не будет читать. А вот хорошо структурированный, литературно написанный и очевидно корректный-по-построению код — лучшая спецификация и лучшая документация самому себе*. Обеспечение возможности — центральная задача при проектировании языков программирования.
...
## Деконструкция объектно-ориентированного языка программирования
-----------------------------------------------------------------

Ещё в конце 1950ых началась разработка языков спецификации, в рамках которых можно прямо и или опосредованно рассуждать об объектах, и выражать условия корректности интерактивных программ, такие, как например deadlock freedom и race freedom. Эта линия исследований увенчалась разработкой TLA⁺, активно используемым в индустрии с блестящим track record по части обнаружения критических багов в важнейших системах Microsoft и Amazon. К сожалению TLA⁺ и все языки спецификации, основанные на схожих принципах, структурно очень далеки от исчисления предикатов, и не приспособлены функционировать в качестве механизма для создания корректных-по-построению интерактивных программ. 

Разработка J.Y. Girard'ом в конце 1980ых-начале 1990ых линейной логики открыла путь к добавлению в исчисление предикатов возможности рассуждать ещё и об объектах. Такие расширения называют Adjoint logic(s), и являются на данный момент областью активных научных исследований, особенно активизировавшейся в последние 5-10 лет, которыми в том числе занимается и автор этого текста. В то время как исчисление предикатов способно выражать __любые__ требования к программам (на строго-функциональных ЯП), имеющим отношения к корректности, выразительность adjoint logics остаётся (tbc)


Ниже я продемонстрирую, как шаг за шагом добавить в чисто-функциональный язык программирования императивные элементы, не теряя возможости строго описывать свойства программ. В примерах я буду использовать вымышленный чисто-функциональный язык с синтаксисом на основе Котлина.

Даже если вы не намереваетесь заниматься формальной верификацией и вообще когда-либо пользоваться таким языком программирования, понимание его устройства поможет лучше понять многие моменты, касающиеся параллелизации, out-of-order execution, а также на многие тонкости, постоянно возникающие в многопоточном программировании.
