...
Будем называть строго-функциональным ядром Kotlin'а вариант языка, все функции представляют из себя чистые функции, а все аргументы, параметры, и все локальные переменные представляют из себя данные.  В частности:
...
* использовать отбитый пробелом правый треугольник `x ▸ f` вместо `x.let f` в качестве оператора применения функции налево `x ▸ f := f(x)`;
