...
Вместо того, чтобы осознать глубокие различия между объектами и данными и конструировать языки программирования с учётом этих различий, при разработке таких ЯП как, например, Java, Python и JavaScript сложилась парадоксальная практика смешения несовместимого и игнорирования нестыковок. Так, например, во всех этих языках и всех языках значения нечисловых типов отождествляют грубо говоря с “ячейками памяти, помеченными как неизменяемые”, и равные значения остаются различимыми при помощи операторов сверки адресов ячеек памяти (`==` в Java, `===` в JavaScript). В результате чего мы можем иметь две разных равных (это не опечатка) строки `"Hello world"`. В Python'е можно иметь два разных числа 100000000000000000000, но не 1000.

Смешение несовместного приводит к нестыковкам и противоречиям, которые нужно тщательно избегать. Однако для этого нужно понимать, где они могут возникнуть, что требует очень глубокого знания технических деталей устройства этих языков.

Python принято считать очень простым в изучении языком. Так ли это? Ответ зависит от того, что мы считаем удовлетворительным результатом изучения языка. Все мы с раннего детства знакомы с рецептами, инструкциями по сборке шкафов и пользованию электроприборами — думать в терминах последовательных действиях над объектами нам _привычно_. Эта ментальная модель позволяет быстро начать писать на языках вроде Python'а код, который, если повезёт заработает. Но эта ментальная модель не позволяет предсказать, где именно 

Если задача в том, чтобы уметь написать недлинную программу для собственных нужд, 

Вероятность случайно наткнуться на нестыковку при написании короткой несложной программы очень мала, так что если цель научиться писать на Python небольшие программы на один раз, которые чаще всего работают — питон

Если задача состоит в том, чтобы писать предсказуемо работающие программы, то каждый из вышеописанных языков чудовищно сложен в изучении, потому что 
...
Добавление в исчисление предикатов возможности рассуждать ещё и об объектах — на данный момент предмет активных исследований (см. Adjoint logic), и удобный инструментарий по этой части ещё не сформировался.

Аналогично, все мы с раннего детства знакомы с рецептами, инструкциями по сборке шкафов и пользованию электроприборами — думать в терминах объектов и последовательных действиях над ними нам _привычно_, в то время как думать в терминах значений и их преобразований привычно только профессиональным математикам. Поэтому императивный объектно-ориентированные языки воспринимаются как нечто простое и легко понятное, в то время как чисто-функциональные (purely functional) языки программирования как нечто очень сложное. И это верно в том смысле, что функциональные языки сложнее изучить. Дело тут в том, что все 
