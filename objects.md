...
В мире императивного программирования (и моделирования физической реальности) центральным понятием
является объект. Скажем, мутабельная переменная, содержащая целочисленное значение. Это такой “ящик”
где лежит число. В двух разных ящиках могут лежать равные числа, но ящики при этом остаются разными
— в одном из них можно значение поменять, а в другом оно останется таким как было.

Центральное понятие чистых языков программирование — понятие значения в математическом смысле,
например натуральное число 2.

Про объекты мы можем говорить “тот же самый” (same) или “другой” (other). Это называют наличием
сущности (identity). Объекты также имеют lifecycle — они в какой-то момент могут пораждаться, в
какой-то момент исчезать, хотя какие-то объекты возможно “всегда были” и/или “всегда будут”.

Про значения можно говорить “равные” (equal, identifiable) или “различные” (distinct, discernable),
но понятия “тот же самый” или “другой” к ним неприменимы, и у них нет никакого lifecycle, про них
даже нельзя сказать что они “всегда были и будут” — они просто вообще вне времени и не имеют
сущности. Они — информация, отделённая от ностеля.
...
```
def main(args : List[String], STDOUT : !(String ⊸ Unit) ):
  for
    i <- args
  do
    STDOUT(i ++ "\n")
...
```
def main(args : List[String], TTY : !(String ⊸ String) )
```
...
результатов, которые мы будем обозначать через =r= и =e= соответственно. 
...
```
def main(args : List[String], TTY1 : !(String ⊸ String),
                              TTY2 : !(String ⊸ String) )
```
