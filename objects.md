...
Существуют также sharable resources — это объекты, имеющие специальную команду расщипления, позволяющую одновременно оставить ресурс у себя дать доступ к нему вызываемой процедуре. Один класс таких ресурсов это Queue и Database, с одной стороны в них можно класть новые сообщения/посылать им updates, с другой стороны корутина может быть подписана на новые сообщения/подписана на queries. В терминах таких sharable ресурсов описывается actor model и join calculus, а для описания условий корректности при работе с ними используется понятие eventual consistency.
Другой класс sharable resources — Heap и CoroutineContext, они позволяют порождать внутри себя новые подобъекты (variables в Heap, workers в CoroutineContext), на которые в свою очередь можно получить и раздать дополнительные views с теми или иными capabilities: так например, можно передать куда-то ссылку на переменную только для чтения, а можно для чтения и записи. Для рассуждений о них понадобится сепарационная логика, о чём я ещё напишу ниже.
...
Для некоторых объектов, таких как Heap и CoroutineContext, возможно определить понятие изооированных подобъектов, описываемых в терминах partial commutative monoids, и тогда соответствующая монада Дейкстры будет снабжена богатой дополнительной структурой, позволяющей выражать weakest precondition-based concurrent separation logic.