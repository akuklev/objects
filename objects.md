...
* использовать dot-notation (`point.x`) только для properties, а для методов будем использовать правый треугольник вместо точки:
```kotlin
files ▸filter 
  it.type = "image/png" &&
  it.size > 0 
...
Stratified Kotlin — надмножество Pure Kotlin, в котором мы допускаем использование команд и существование псевдовыражений.
