...
```kotlin
val a := 60 · 24
print("Number of minutes in a day is " ++ a)
```

Выражения описывают как вычислить значение, и взаимозаменимы с этим значением. Выражения представляют из себя деревья применения функций к значениям. Скажем, `60 · 24` предписывает применить функцию `(·)` к значениям 60 и 24, `"Number of minutes in a day is " ++ a` аналогичным образом применяет функцию конкатенации строк `(++)`.
...
§ Зачем различать объекты и данные?
-----------------------------------
