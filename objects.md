...
Всякую функцию `someLengthyCalculation(x : X) : Y` мы можем превратить в корутину

```kotlin
suspend fun performLengthyCalculation(x : X, cont : Continuation<Y>) {
  val result = someLengthyCalculation(x)
  !cont(result)
}
```
...
```kotlin
context(Log)
suspend fun foo(x : X) : Y {
  !trace("Executing foo.....")
  suspendCoroutine {
    val y := !performLengthyCalculation(x, it)
  }
