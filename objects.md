...
Если в языке есть инструкции, заменяемость на значение уже не работает. Для примера предположим, что в нашем языке есть оператор под названием `!rand`, недетерминистски генерирующий случайное целое число. Рассмотрим следующие две программы, которые должны бы быть эквивалентны:
```kotlin
val n := !rand
n + n
```
...
Программой (routine) мы будем называть последовательность инструкций, возможно ветвлениями и циклами. Инструкции (statements) внешне похожи на выражения, но их контексты могут содержать кроме значений ещё и операторы, такие как вышеприведённые `!rand` и `!trace`. С символами операторов нужно обращаться совсем по-другому, чем с иными символами, т.к. не работает подстановочность, специально для этого мы начинаем названия всех таких символов с восклицательного знака. Хорошо знакомые всем программистам инструкции `!throw` и `!return` тоже можно рассматривать как !-операторы.
...
Мы говорим, что оператор `!foo : ReqT ⊸ RespT` детерминирован, если будучи вызван с одинаковым
...
запись в независимые потоки вывода (например, `!print` и `!trace`) независима, что мы можем
выразить через `{!print(x); !trace(y)} =e= {!trace(y); !print(x)}`.
...
```kotlin
fun main(args : List<String>, tty1 : Tty,
                              tty2 : Tty)
```
