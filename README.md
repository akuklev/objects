...
Чистые функции это обобщения таблиц ключ-значение. В случае конечного типа `X`, например, `X = {0, 1, 2, 3}`, чистые функции `(X)-> Y` это в точности полные таблицы ключ-значение `Map<X, Y>`. Чтобы особенно чётко продемонстрировать, что чистые функции это не объекты, а данные, отметим что одна чистуа функция может примать другую чистую функцию в качестве аргумента. Такова, например, функция `applyElementwise(transformation : (X)-> Y, list : List<X>) : List<Y>`. Сама эта функция тоже всего лишь значение типа `((X)-> Y, List<X>)-> List<Y>`.
