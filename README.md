...
Канонические примеры значений — числа, строки (конечные строки букв заданного алфавита), конечные `List<Int>` или бесконечные `Seq<T>` списки чисел, конечные `Map<X, Y>` и потенциально бесконечные `(X)-> Y` таблицы ключ-значение, также называемые **чистыми функциями**. Подчеркнём, что чистые функции, сопоставляющие каждому значению `x : X` значение типа `Y` это именно значения, а не объекты. В частности одна чистая функция может примать другую чистую функцию в качестве аргумента. Такова, например, функция `map(f : (X)-> Y, xs : List<X>)`, применяющая функцию `f` поэлементно ко всем элементам списка `xs`. Сама функция `map` — тоже всего лишь значение типа `((X)-> Y, List<X>)-> List<Y>`.
